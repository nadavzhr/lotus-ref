...
export function TopMenuBar({ showChat, onToggleChat, onOpenFile }: TopMenuBarProps) {
      ...
      <Menubar className="border-none bg-transparent shadow-none h-8 p-0">
        <MenubarMenu>
          ...
          <MenubarContent>
            ...
            <MenubarItem onClick={() => store.saveDocument()}>
              Save <MenubarShortcut>Ctrl+S</MenubarShortcut>
            </MenubarItem>
            <MenubarSeparator />
            <MenubarItem
              onClick={() => {
                if (store.activeDocId) store.closeDocument(store.activeDocId)
              }}
              disabled={!store.activeDocId}
            >
              Close Document
            </MenubarItem>
          </MenubarContent>
        ...
        <MenubarMenu>
          ...
          <MenubarContent>
            <MenubarItem onClick={() => store.undoAction()}>
              Undo <MenubarShortcut>Ctrl+Z</MenubarShortcut>
            </MenubarItem>
            <MenubarItem onClick={() => store.redoAction()}>
