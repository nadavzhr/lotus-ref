...
from doc_types.af import AfLineData, validator
...
class TestAfValidatorDomain:
    ...
    def test_af_at_boundaries(self):
        assert validator.validate(AfLineData(net="vdd", af_value=0.0, is_em_enabled=True)).is_valid
        assert validator.validate(AfLineData(net="vdd", af_value=1.0, is_em_enabled=True)).is_valid
